...
namespace QSCustomer.Extensions
{
    public class RenderHtmlTableExtensions
    {
        ...
        int i_emp = 0;
        int s_emp = 0;
        public RenderHtmlTableExtensions(IHubContext<HomeHub> context, IUnitOfWork uow, Claim claim, IHttpContextAccessor accessor)
        {
            _uow = uow;
            _context = context;
            _claim = claim;
            ...
            <div class='pb-lg-5'>
                ...
                if (_report._PartNrTanimlari.Count() > 21)
                {
                    for (int i = 0; i < 21; i++)
                    {
                        ...
                        {
                            if(_report._PartNrTanimlari[i].partNrTanimi != "")
                ...
                if (_report._ProjeHataTanim.Count() > 14)
                {
                    for (int i = 0; i < 12; i++)
                    {
                        ...
                        {
                            if(_report._ProjeHataTanim[i].hataTanimi != "")
...
";
                ...
                string SECTION_FOUR = "";
                ProjectDetLength = _report._ProjectDetails.Count();
        ...
        public async Task<string> ReportHeader(PdfReport _report)
        {
            int productRef_count = 0, projectDetails_count = 0;
            double spent = (_report._SpentHours * 100) / 100;
            double OverTime100 = (_report._OverTime100Total * 100) / 100;
